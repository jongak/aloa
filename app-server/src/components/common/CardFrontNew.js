...
const CardFrontNew = function ({ setIsLoading, style, divRef }) {
  const imgRef = useRef();
  ...
  const isTitle = useSelector((state) => state.isTitle);
  const isLevel = useSelector((state) => state.isLevel);
  const framePreset = useSelector((state) => state.framePreset);
